...
export async function fetchJoinableMatchByInvite(inviteCode, uid) {
  ...
  try {
    ...
    if (waitingMatches && waitingMatches.length > 0) {
      console.log('[liveApi] Found waiting match:', waitingMatches[0]);
      return { battle: waitingMatches[0], path: 'waiting', error: null };
    }
    ...
    if (uid) {
      ...
      if (activeMatches && activeMatches.length > 0) {
        console.log('[liveApi] Found active match where user is participant:', activeMatches[0]);
        return { battle: activeMatches[0], path: 'activeSelf', error: null };
      }
    }
