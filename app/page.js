  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-parchment">
      ...
      <div className="mt-12 p-8 bg-papyrus border border-sepia/20 rounded-2xl shadow-lg w-full max-w-sm flex flex-col gap-4">
        ...
        <button onClick={() => setView('daily')} className="w-full px-6 py-3 bg-gold-rush text-ink font-bold text-lg rounded-lg hover:bg-amber-600 transition-colors shadow-md">
          Daily Challenge
        </button>
...
function ProfileView({ setView, session }) {
    ...
    useEffect(() => {
        async function getProfileData() {
            ...
            if (user) {
                const { data: profileData, error: profileError } = await supabase.from('profiles').select('username, avatar_url').eq('id', user.id).single();
                if (profileError) console.error('Error fetching profile:', profileError);
                else setProfile(profileData);

                const { data: scoresData, error: scoresError } = await supabase.from('scores').select('score').eq('user_id', user.id).order('created_at', { ascending: false });
...
function ChallengeView({ setView, session, setActiveChallengeId }) {
    ...
    const incomingChallenges = challenges.filter(c => c.opponent_id === currentUserId && c.status === 'pending' && c.opponent_score === null);
    const outgoingChallenges = challenges.filter(c => c.challenger_id === currentUserId && c.status === 'pending');
...
// --- GAME VIEW COMPONENT ---
function GameView({ setView, challengeId = null, session, onChallengeComplete, dailyPuzzle = null, onDailyStepComplete = null }) {
  ...
  useEffect(() => {
    ...
    fetchPuzzleData();
  }, [challengeId, gameKey, dailyPuzzle]);
  ...
  const handleGuessSubmit = async () => {
    ...
    if (challengeId) {
        ...
        // Daily challenge logic is handled in DailyChallengeView
    } else {
  ...
  const handlePlayAgain = () => {
    ...
    else if (dailyPuzzle) onDailyStepComplete(results.finalScore);
    else {
  ...
  return (
    <main className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
      <header className="mb-8 text-center relative"><button onClick={() => setView(challengeId ? 'challenge' : dailyPuzzle ? 'daily' : 'menu')} className="absolute left-0 top-1/2 -translate-y-1/2 px-4 py-2 bg-sepia-dark text-white font-bold rounded-lg hover:bg-ink transition-colors shadow-sm">&larr; Back</button><div><h1 className="text-5xl font-serif font-bold text-gold-rush">HistoryClue</h1><p className="text-lg text-sepia mt-2">{dailyPuzzle ? `Daily Challenge - Puzzle ${dailyPuzzle.step}` : challengeId ? 'Challenge Match' : 'Endless Mode'}</p></div></header>
...
function DailyChallengeView({ setView, session, setActiveDailyPuzzle }) {
  // This is a placeholder for the Daily Challenge UI
  // In the next step, we will build this out fully.
  return (
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 min-h-screen">
          <header className="mb-8 text-center relative">
              ...
              <h1 className="text-5xl font-serif font-bold text-gold-rush">Daily Challenge</h1>
          </header>
          <div className="text-center p-8 bg-papyrus rounded-lg shadow-lg border border-sepia/20">
              <p className="text-2xl font-serif text-ink">The Daily Challenge is coming soon!</p>
              <p className="text-sepia mt-4">Check back later to test your skills against a new set of puzzles each day.</p>
          </div>
      </div>
  );
}
...
export default function Page() {
    ...
    if (view === 'game') return <GameView setView={setView} challengeId={activeChallengeId} session={session} onChallengeComplete={onChallengeComplete} dailyPuzzle={activeDailyPuzzle} onDailyStepComplete={() => { /* Logic to handle step completion */ }} />;
