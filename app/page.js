...
function GameView({ setView, challengeId = null, session }) {
  ...
  const handleGuessSubmit = async () => {
    ...
    if (challengeId) {
        const { data: challenge, error: fetchError } = await supabase.from('challenges').select('challenger_id, opponent_id, challenger_score').eq('id', challengeId).single();
  ...
  return (
    <main className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
      <header className="mb-8 text-center relative"><button onClick={() => setView(challengeId ? 'challenge' : 'menu')} className="absolute left-0 top-1/2 -translate-y-1/2 px-4 py-2 bg-sepia-dark text-white font-bold rounded-lg hover:bg-ink transition-colors shadow-sm">&larr; Back</button><div><h1 className="text-5xl font-serif font-bold text-gold-rush">HistoryClue</h1><p className="text-lg text-sepia mt-2">{challengeId ? 'Challenge Match' : 'Endless Mode'}</p></div></header>
