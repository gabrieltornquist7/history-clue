  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-parchment">
      <div className="p-8 bg-papyrus border border-sepia/20 rounded-2xl shadow-lg w-full max-w-sm">
        ...
        <p className="text-center text-sepia mb-6">Sign in or create an account.</p>
        <form className="space-y-4">
          ...
          <div className="flex gap-4 pt-4">
            <button onClick={handleLogin} className="flex-1 px-4 py-2 bg-sepia-dark text-white font-bold rounded-lg hover:bg-ink transition-colors shadow-md" disabled={loading}>
              {loading ? <span>Loading...</span> : <span>Sign In</span>}
            </button>
            <button onClick={handleSignUp} className="flex-1 px-4 py-2 bg-gold-rush text-ink font-bold rounded-lg hover:bg-amber-600 transition-colors shadow-md" disabled={loading}>
